...
| `generate-lb-guides.js` | `lb-coxs-bazar-trainer-guide.docx` | For NRC's **master educators**, who induct the facilitators. Three layers: Amala trains the master educators (the `ttt-cb-*` workshop pack), they induct the facilitators from this guide, the facilitators deliver from the Educator Guide. The two inductions' structure is rendered from `qualification.inductions` in the programme YAML; the block-by-block shape is authored in `INDUCTION_SHAPES` in the generator, and `checkInductionShapes()` fails the run if an induction is added, removed or renamed in the YAML without its shape being updated. |
